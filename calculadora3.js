...
function calcularDescuento(){
	...
	// superposición de información al momento de la interacción
		...
		if (!precioInteger || !cuponString){
			...
			function filtrarCupon(cupon){
				return cupon.nombre == cuponString;
			}

			cuponElegido = cupones.filter(filtrarCupon);
			console.log(cuponElegido);

			for (let i = 0; i < cuponElegido.length; i++) {
				resultadoFinal.innerHTML = "Su precio con dcto. es " + (precioInteger*(100-(cuponElegido[i].descuento)))/100 + " $";
				dctoFinal.innerHTML = "Obtuvo un dcto. de" + " " + (cuponElegido[i].descuento) + " "+ "%";
				console.log(cuponElegido[i].descuento);
